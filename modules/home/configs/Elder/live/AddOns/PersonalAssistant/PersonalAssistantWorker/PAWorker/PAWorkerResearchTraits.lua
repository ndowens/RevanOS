...
local function StartResearchTrait(hasDeconstructedOrRefinedBefore)
	...
	if PAMF.PAWorker.getAutoExitCraftingSetting() and (count ~= 1000 or hasDeconstructedOrRefinedBefore) then
	       if count == 1000 then
		       zo_callLater(function() SCENE_MANAGER:ShowBaseScene() end, 1000) --- exit crafting table
		   else
		       if count > 3000 then count = 3000 end -- can't research more than 3 at once so...
		       zo_callLater(function() SCENE_MANAGER:ShowBaseScene() end, count+1000) --- exit crafting table
		   end
