...
local function LLC_CraftProvisioningItemByRecipeIndex(self, recipeListIndex, recipeIndex, timesToMake, autocraft, reference)
	...
	if resultLink == "" then
		resultLink = getItemLinkFromItemId(itemId)
	end
	local request = 
	{
		["recipeId"] = nil,
...
local function getRecipeItemIdFromResultItemId(resultId)
  ...
  local recipeId = resultId + offset
  if recipeId then
    -- Item is in the list!
...
local function CompileProvisioningRequirements(request)
	...
	if not GetRecipeInfo(request.recipeListIndex, request.recipeIndex) then -- is recipe known?
		-- if it is not known, we need to use backup ingredient info method
		local recipeItemLink = getRecipeItemIdFromResultItemId(GetItemLinkItemId(request.resultLink))
		numIngredients = GetItemLinkRecipeNumIngredients(recipeItemLink)
		for i = 1, numIngredients do
			local id = GetItemLinkItemId(GetItemLinkRecipeIngredientItemLink(recipeItemLink, i))
			if id and id~=0 then
				local name, amountHeld, required = GetItemLinkRecipeIngredientInfo(recipeItemLink, i)
				requirements[id] = required * request.timesToMake
			end
		end
		return requirements
	end
